...
group.add_argument('--adv-train', action='store_true', default=True,
                    help='weather adversarial traning')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args, attack,
...
class GradCam:
    ...
    def __call__(self, input, index=-1):
        ...
        one_hot = np.zeros((1, output.size()), dtype=np.float32)
        print(one_hot.shape)
        one_hot[0][index] = 1
        ...
        self.model.segmentation_head.zero_grad()
        one_hot.backward(retain_graph=True)
        # print("the self.extractor.get_gradients() size is ", self.extractor.get_gradients()[-1].size())
        ...
        # cam = np.maximum(cam, 0.000001)
        # print("---------",cam.shape)
        cam = cv2.resize(cam, (640, 480))
        #cam = cam - np.min(cam)
        #cam = cam / np.max(cam)
        # X_cam = np.maximum(X_cam, 0)
        X_cam = cv2.resize(X_cam, (640, 480))
...
def amat_attack(model, inputs, labels, eps, alpha, num_steps):

    # Clone inputs and set them as requires_grad
    ...
    loss_fun = nn.L1Loss()
    loss_fun2 = nn.L2Loss()
    f1_fea = model.forward_features(inputs)

    # Perform num_steps of gradient descent
    for step in range(num_steps):
        # Forward pass
        outputs = model(perturbed_inputs)
        [cam, _] = grad_cam(perturbed_inputs, labels)

        # Compute the loss
        loss1 = torch.nn.CrossEntropyLoss()(outputs, labels)
        loss2 = loss_fun(cam)
        f2_fea = model.forward_features(perturbed_inputs)
        loss3 = loss_fun2(f1_fea[-3],f2_fea[-3])
        total_loss = loss1 + 0.6*loss2 + 2*loss3
        # Backward pass
        total_loss.backward()

        # Add perturbation to the inputs
        perturbed_inputs.data = perturbed_inputs.data + alpha * perturbed_inputs.grad.sign()

        # Clamp the perturbed inputs to lie within the epsilon ball
        perturbed_inputs.data = torch.max(torch.min(perturbed_inputs.data, inputs + eps), inputs - eps)

        # Zero the gradients
        perturbed_inputs.grad.data.zero_()
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args, attack,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        ## attack
        # Set hyperparameters for AM-AT adversarial training
        eps = 0.189  # maximum perturbation
        alpha = 0.0284  # step size
        num_steps = 10  # number of gradient descent steps
        input = amat_attack(model, input, target, eps, alpha, num_steps)
        with amp_autocast():
            output = model(input)
