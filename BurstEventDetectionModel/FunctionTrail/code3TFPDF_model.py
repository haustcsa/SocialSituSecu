...
from FuntionCompare.twoSocial_tfpdf.code5jieba_fenci import fenci_word
...
class TFPDF():
    def __init__(self, news_data: pd.DataFrame):
        ...
        self.tags = self.get_tags(self.contents)
        # print("word_list:\n", self.word_list)
    ...
    def get_word_tag(self, word):
        sum_tag = len(self.tags)
        # print("self.tags", self.tags)
        tag_dict = {}
        for i in self.tags:  # 遍历整个列表
            if i in tag_dict.keys():  # 判断当前key是否已经存在
                tag_dict[i] = tag_dict[i] + 1  # 在当前key的个数上加 1
            else:
                tag_dict[i] = 1  # 当前key第一次出现
        # print("tag_dict", tag_dict)
        if word in tag_dict.keys():
            tag = round((tag_dict[word]+1) / sum_tag, 7)
        else:
            tag = round(1 / sum_tag, 7)
        return tag
