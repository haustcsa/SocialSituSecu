...
def get_result100(V):
    compare_datas = []
    compare_files = os.listdir('../results/word_fenci')
    # print(compare_files)
    for compare_file in compare_files[11:]:
        compare_data = []
        compare_path = '../results/word_fenci/' + compare_file
    ...
    path = '../results/word_zmerge/bursty_v{0}.txt'.format(V)
...
def compute(data, compare_datas, n):
    ...
    data[-1] = data[-1].split('\n')[0]
    headers = data
    f = open('../results/final_result/' + n + '_input.csv', 'w', encoding='utf8', newline='')
    writer = csv.writer(f)
    # 表头
