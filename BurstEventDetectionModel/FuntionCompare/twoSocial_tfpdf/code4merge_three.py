...
def read_files():
    # 读取词频增长率文件夹中的文件
    # increment_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\results\word_social')
    tfpdf_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\results\word_NTFPDF')
    # # TODO 果真不按顺序进行输出，所以。。。
    ...
    #     f = open(r'D:\workspace\pycharm\PaperTrail\results\word_increment' + '\\' + increment_file, 'r', encoding='utf-8')
    #     data = json.load(f)
    #     increment_data.append(data)
    tfpdf_data = []
    for tfpdf_file in tfpdf_files:
        f = open(r'D:\workspace\pycharm\PaperTrail\results\word_NTFPDF' + '\\' + tfpdf_file, 'r', encoding='utf-8')
        data = json.load(f)
...
def compute_butsty(tfpdf_data, V):
    ...
    print("two social tfpdf new_a_list", len(new_a_list), new_a_list)
