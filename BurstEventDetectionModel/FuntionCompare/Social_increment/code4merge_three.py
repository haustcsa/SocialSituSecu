...
def read_files():
    # 读取词频增长率文件夹中的文件
    increment_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\results\word_increment')
    # # TODO 果真不按顺序进行输出，所以。。。
    # # increment_files.sort(key=lambda x: int(x[:x.find("_")]))  # 按照前面的数字字符排序
    # print("再次验证文件是不是按顺序的", len(increment_files), increment_files)
    # tfpdf_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\results\word_social')
    # TODO 对比实验
    ...
    #     f = open(r'D:\workspace\pycharm\PaperTrail\results\word_social' + '\\' + tfpdf_file, 'r', encoding='utf-8')
    #     data = json.load(f)
    ...
    for i_data, j_data in zip(increment_data, social_data):
