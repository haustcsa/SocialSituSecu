...
def get_result100(V):
    compare_datas = []
    compare_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\results\word_fenci')
    # print(compare_files)
    for compare_file in compare_files[11:]:
        compare_data = []
        compare_path = r'D:\workspace\pycharm\PaperTrail\results\word_fenci' + '\\' + compare_file
        print("compare_path ", compare_path)
        f = open(compare_path, encoding='utf-8')
        for line in f:
...
def compute(data, compare_datas, n):
    ...
    data[-1] = data[-1].split('\n')[0]
    headers = data
    f = open(r'D:\workspace\pycharm\PaperTrail\CompareTrail\ZhangqiangCISC\final_result' + '\\' + n + '_input.csv', 'w', encoding='utf8', newline='')
    writer = csv.writer(f)
    # 表头
