...
def read_files():
    ...
    # # TODO 果真不按顺序进行输出，所以。。。
    # TODO 对比实验
    tfidf_files = os.listdir(r'../CompareTrail/ZhangElectric/TFIDF_word')
    ...
    tfpdf_data = []
    for tfpdf_file in tfidf_files:
        f = open(r'../CompareTrail/ZhangElectric/TFIDF_word/' + tfpdf_file, 'r', encoding='utf-8')
        data = json.load(f)
...
def compute_butsty(tfpdf_data, V):
    ...
    file_path = '../CompareTrail/ZhangElectric/TOP100_value/bursty_value_v{0}.json'.format(V)
