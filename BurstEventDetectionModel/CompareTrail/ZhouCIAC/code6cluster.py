...
def cluster(t, criterion):
    news_data_files = os.listdir(r'D:\workspace\pycharm\PaperTrail\CompareTrail\ZhouCIAC\final_result')
    path_cluster = r'D:\workspace\pycharm\PaperTrail\CompareTrail\ZhouCIAC\distance_cluster\{0}_cluster.txt'.format(t)
    f_cluster = open(path_cluster, 'w', encoding='utf-8')
    for news_data_file in news_data_files:
        path = r'D:\workspace\pycharm\PaperTrail\CompareTrail\ZhouCIAC\final_result' + '\\' + news_data_file
...
if __name__ == '__main__':
    ts = [1.2, 1.25, 1.3, 1.35, 1.4, 1.5, 1.6, 1.7]
    # criterion = 'maxclust'
    criterion = 'distance'
    for t in ts:
        cluster(t, criterion)