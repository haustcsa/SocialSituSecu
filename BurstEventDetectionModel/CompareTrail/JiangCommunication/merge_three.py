...
def read_files():
    # 读取词频增长率文件夹中的文件
    increment_files = os.listdir(r'../results/word_increment')
    # # TODO 果真不按顺序进行输出，所以。。。
    # TODO 对比实验
    dfidf_files = os.listdir(r'../CompareTrail/JiangCommunication/word_dfidf')
    social_files = os.listdir(r'../results/word_social')
    increment_data = []
    for increment_file in increment_files:
        f = open(r'../results/word_increment/' + increment_file, 'r', encoding='utf-8')
        data = json.load(f)
        increment_data.append(data)
    tfpdf_data = []
    for tfpdf_file in dfidf_files:
        f = open(r'../CompareTrail/JiangCommunication/word_dfidf/' + tfpdf_file, 'r', encoding='utf-8')
        data = json.load(f)
        tfpdf_data.append(data)
    social_data = []
    for social_file in social_files:
        f = open(r'../results/word_social/' + social_file, 'r', encoding='utf-8')
        data = json.load(f)
...
def compute_butsty(tfpdf_data, V):
    ...
    file_path = '../CompareTrail/JiangCommunication/TOP100_value/bursty_value_v{0}.json'.format(V)
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(new_a_list, f, ensure_ascii=False, indent=4)
    f.close()
    print("bursty_value 存储完成")

    path = r'../CompareTrail/JiangCommunication/TOP100/bursty_v{}.txt'.format(V)
