...
if __name__=='__main__':
    ...
    pornfile.close()
    #加载涉政文本
    politicalfile = open("../political.txt",encoding='UTF-8')
    for line in politicalfile :
        politicalNum = politicalNum + 1
        dict1 = {}
        # dict1['type'] = 'political'
        dict1['content'] = line
        sensitivityList.append(dict1)
    politicalfile.close()
    #加载涉恐模型
    terrorismfile = open('../terrorism.txt',encoding='UTF-8')
    for line in terrorismfile :
        terrorismNum = terrorismNum + 1
        dict1 = {}
        # dict1['type'] = 'terrorism'
        dict1['content'] = line
        sensitivityList.append(dict1)
    terrorismfile.close()
    ...
    for textdata in normalList:
        ...
        for resultModel in result :
            ...
            j = j+1
    print("**********************最终检测结果****************************")
    print("涉黄文本检测的准确率为：{}".format(pornCount/pornNum))
    print("涉政文本检测的准确率为：{}".format(politicalCount/politicalNum))
    print("涉暴文本检测的准确率为：{}".format(terrorismCount/terrorismNum))
    print("普通文本检测的准确率为：{}".format(normalCount/(len(normalList))))
