...
# MODELS_DIR = "models"
# MODELS_DIR = "G:\DFDC\model_dir"
# BASE_DIR = "../../deep_fakes"
BASE_DIR = "F:\FFtest"
# DATA_DIR = os.path.join(BASE_DIR, "dataset")
DATA_DIR = os.path.join(BASE_DIR, "dataset")
TEST_DIR = os.path.join(DATA_DIR, "test_set")
OUTPUT_DIR = "F:\FFtest/test_resoult"
#OUTPUT_DIR = os.path.join(MODELS_DIR, "tests")

TEST_LABELS_PATH = os.path.join(BASE_DIR, "dataset/dfdc_test_labels1.csv")
...
def save_roc_curves(correct_labels, preds, model_name, accuracy, loss, f1):
    ...
    plt.title('ROC curve')
    plt.legend(loc='best')
    plt.savefig(os.path.join(OUTPUT_DIR, model_name + "_" + opt.dataset + "_acc" + str(accuracy * 100) + "_loss" + str(
        loss) + "_f1" + str(f1) + ".jpg"))
    plt.clf()
...
def read_frames(video_path, videos):
    ...
    # frames_interval = int(frames_number / opt.frames_per_video)使用默认值30
    #+rames_interval = 1
    ...
    if frames_interval > 0:
        for key in frames_paths_dict.keys():
            ...
            # frames_paths_dict[key] = frames_paths_dict[key][:opt.frames_per_video]使用默认值30,截取前30个帧
            frames_paths_dict[key] = frames_paths_dict[key][:30]

    # Select N frames from the collected ones
    video = {}
    # 11
    with open('F:\DeepFake\Vit_CNN\cross-efficient-vit\configs/architecture.yaml',
              'r') as ymlfile:
        config_m = yaml.safe_load(ymlfile)

    for key in frames_paths_dict.keys():
        for index, frame_image in enumerate(frames_paths_dict[key]):
            transform = create_base_transform(config_m['model']['image-size'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path',
                        default='F:\Com_T_pth/efficientnet_checkpoint16_Deepfakes',
                        type=str, metavar='PATH',
                        help='Path to model checkpoint (default: none).')
    parser.add_argument('--dataset', type=str, default='Deepfakes',
                        help="Which dataset to use (Deepfakes|Face2Face|FaceShifter|FaceSwap|NeuralTextures|DFDC)")
    parser.add_argument('--max_videos', type=int, default=-1,
                        help="Maximum number of videos to use for training (default: all).")
    parser.add_argument('--config', type=str,
                        default='F:\DeepFake\Vit_CNN\cross-efficient-vit\configs/architecture.yaml',
    ...
    if os.path.exists(opt.model_path):
        model = CrossEfficientViT(config=config_m)
        model.load_state_dict(torch.load(opt.model_path)) #转换为CPU
        # model.load_state_dict(torch.load(opt.model_path, map_location=torch.device('cpu')), strict=True)
        # model.eval()
        # model = model.cuda()
    ...
    test_counter = collections.Counter(image[1] for image in videos)
    print('此次测试图片数量',test_counter)
    ...
    print("correct_test_labels",correct_test_labels)
