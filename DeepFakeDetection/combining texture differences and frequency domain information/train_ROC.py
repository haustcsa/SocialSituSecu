...
def read_frames(video_path, train_dataset, validation_dataset):
    ...
    if TRAINING_DIR in video_path:
        ...
        if "Original" in video_path:
            ...
            for json_path in glob.glob(r'G:/DFDC/dfdc_train_Part_49/dataset/metadata/metadata.json'):
    ...
    with open('G:/Com_efficient_config/architecture.yaml', 'r') as ymlfile:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_epochs', default=17, type=int,
                        help='Number of training epochs.')
    parser.add_argument('--workers', default=8, type=int,
                        help='Number of data loader workers.')
    parser.add_argument('--resume', default='F:\Com_T_pth\pthDFDC/efficientnet_checkpoint16_DFDC',
                        type=str, metavar='PATH',
                        # parser.add_argument('--resume', default='', type=str, metavar='PATH',
                        help='Path to latest checkpoint (default: none).')
    parser.add_argument('--dataset', type=str, default='DFDC',  # All
    ...
    for t in range(starting_epoch, opt.num_epochs + 1):
        ...
        # 创建一个DataFrame，将标签和预测值作为列
        # df = pd.DataFrame({
        #     'Actual': all_labels,
        #     'Predicted': all_pre
        # })
        # # 将DataFrame保存为CSV文件，不保存索引列
        # df.to_csv('H:\classification_results.csv', index=True)



        # 绘制ROC曲线
        fpr, tpr, th = metrics.roc_curve(all_labels, all_pre)
        plt.plot(fpr, tpr, label="Model_" + '测试' + ' (area = {:.3f})'.format(auc_score))
        ...
        # 将DataFrame保存为CSV文件，不保存索引列
        # df.to_csv('H:\classification_results.csv', index=False)
