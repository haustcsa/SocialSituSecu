...
from img_dct import FAD_Head




# 定义模型
class Conv2d_adc(nn.Module):
    def __init__(self, in_channels=3, out_channels=32, kernel_size=7, stride=2,
                 padding=3, dilation=1, groups=1, bias=False, theta=0.7):
    ...
    def forward(self, x):
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            kernel_sum = self.conv.weight.sum(2).sum(2)
            kernel_diff = kernel_sum / self.kernel_area
...
class ImageEmbedder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # 将efficient中的第一层卷积替换为ADC卷积
        self.efficient_net._conv_stem = Conv2d_adc()
...
class ImageEmbedder_lg(nn.Module):
    def __init__(
            self,
            *,
            dim,
        ...
        # todct = FAD_Head(224)
        self.to_dct = FAD_Head(224)

    def forward(self, img):

        image = self.to_dct(img)

        x = self.efficient_net.extract_features_at_block(image, self.efficient_block)
        x = self.to_patch_embedding(x)
        b, n, _ = x.shape
...
class CrossEfficientViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.sm_image_embedder = ImageEmbedder_lg(dim = sm_dim, image_size = image_size, patch_size = sm_patch_size, dropout = emb_dropout, efficient_block = 16, channels=sm_channels)
        ...
        self.lg_image_embedder = ImageEmbedder_lg(dim = lg_dim, image_size = image_size, patch_size = lg_patch_size, dropout = emb_dropout, efficient_block = 1, channels=lg_channels)
