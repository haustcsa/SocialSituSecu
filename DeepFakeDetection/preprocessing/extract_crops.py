...
def extract_video(video, root_dir, dataset):
    try:
        if dataset == 0:
            ...
            bboxes_path = os.path.join('G:\Celeb-DF-v2\Celeb-synthesis', "boxes", os.path.splitext(os.path.basename(video))[0] + ".json")
        else:
            bboxes_path = os.path.join('F:\DeepFake\Data\dfdc_train_part_0', "boxes", get_method_from_name(video), os.path.splitext(os.path.basename(video))[0] + ".json")
        ...
        for i in range(frames_num):
            ...
            ooutput_path = 'G:\Celeb-DF-v2\Celeb-synthesis_output'
            
            #os.makedirs(os.path.join(opt.output_path, id), exist_ok=True)
            os.makedirs(os.path.join(ooutput_path, id), exist_ok=True)
            for j, crop in enumerate(crops):
                cv2.imwrite(os.path.join(ooutput_path, id, "{}_{}.png".format(i, j)), crop)
        if counter == 0:
            print(video, counter)
    except e:
        print("Error:", e)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='G:\Celeb-DF-v2\Celeb-synthesis', type=str,
                        help='Videos directory')
    parser.add_argument('--output_path', default='G:\Celeb-DF-v2\Celeb-synthesis_output', type=str,
