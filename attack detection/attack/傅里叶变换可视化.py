...
def test(attack, e, max_iter):
    ...
    for i, (X, y) in loop:
        ...
        count += 1
        if count == args.count:
            X = torch.fft.fft2(out)
            fshift = torch.fft.fftshift(X)
            out = torch.log(torch.abs(fshift))
            plt.imshow(out.reshape(28, 28, 1).detach().cpu(), 'gray')

            plt.axis('off')
            if args.detect:
                plt.savefig('result/傅里叶变换%s_%s_%s.png' % (args.count, args.detect, args.attack), bbox_inches='tight', pad_inches=0)
            else:
                plt.savefig('result/傅里叶变换%s_%s.png' % (args.count, args.attack), bbox_inches='tight', pad_inches=0)
            plt.show()
            break
