...
def img_transform(model_name):
    if model_name.split('_')[0] == 'EfficientNet':
        if model_name.split('_')[1] == 'v2':
            img_size = {"s": [300, 384],  # train_size, val_size
                        "m": [384, 480],
                        "l": [384, 480]}
            num_model = model_name.split('_')[2]
...
def evaluate(model, data_loader, device, epoch):
    ...
    end_time = time.time()
    print(f'epoch:{epoch}     Image num:{len(data_loader)}     Use Time:{val_time-end_time}s')
    return accu_loss.item() / (step + 1), accu_num.item() / sample_num
